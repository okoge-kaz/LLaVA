...
CHECKPOINT_DIR=/gs/bs/tgh-24IDU/hf-checkpoints/vicuna-13b-v1.3
CHECKPOINT_SAVE_DIR=/gs/bs/tgh-24IDU/checkpoints/llava-13b/pretrain
LLAVA_PROMPT_DATA_PATH=/gs/bs/tgh-24IDU/datasets/vlm/llava/LLaVA-CC3M-Pretrain-595K/chat.json
LLAVA_IMAGE_DIR=/gs/bs/tgh-24IDU/datasets/vlm/llava/LLaVA-CC3M-Pretrain-595K/images
...
export WANDB_NAME="LLaVA-13B-Pre-train-LR_${LR}-WD_${WEIGHT_DECAY}-SEQ_${SEQ_LEN}-EPOCH_${TRAIN_EPOCH}"
...
mpirun -np $NUM_GPUS \
  ...
  --data_path $LLAVA_PROMPT_DATA_PATH \
  --image_folder $LLAVA_IMAGE_DIR \
